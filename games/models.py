...
def check_date(value):
    if value < timezone.now():
        raise ValidationError('現在よりも後に設定してください。')
...
class Game(models.Model):
    ...
    start_at = models.DateTimeField(verbose_name='開始日時', validators=[check_date], default=timezone.now)
...
class SubmittedDeck(models.Model):
    ...
    def get_absolute_url(self):
        return reverse('games:detail', kwargs={'pk': self.game.pk})
